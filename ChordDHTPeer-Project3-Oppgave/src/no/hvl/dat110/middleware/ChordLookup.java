...
public class ChordLookup {
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		for(int i = fingerTable.size() - 1; i >= 0; i--) {
			...
			BigInteger nodeIdPlusOne = node.getNodeID().add(new BigInteger("1"));
			BigInteger keyMinusOne = key.subtract(new BigInteger("1"));
			
			if(Util.computeLogic(fingerStub.getNodeID(), nodeIdPlusOne, keyMinusOne)) {
